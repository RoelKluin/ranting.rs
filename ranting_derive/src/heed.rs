...
// Compile-time entry point for heed!()'s template parsing. The grammar and pattern-building logic
// itself lives in `ranting_core::heed_template` now, shared with `ranting::HeedMatcher::from_template`
// (a runtime constructor for templates that aren't known until runtime, e.g. read from a file) —
// this module's only remaining job is turning a byte-range-carrying `HeedTemplateError` into a
// span-accurate `syn::Error` via `StrLitSlice`, which only exists on this side of the macro/runtime
// seam. See `ranting_core::heed_template`'s module doc for why the shared logic lives there rather
// than being duplicated, or `ranting` depended on directly (illegal: this is a proc-macro crate).

pub(crate) use ranting_core::heed_template::{HeedCapture, HeedCaptureKind};
...
pub(crate) fn compile_heed_template(lit: &StrLit) -> syn::Result<(String, Vec<HeedCapture>)> {
    ...
    ranting_core::heed_template::compile_heed_template(slice.text())
        .map_err(|e| slice.slice(e.range()).error(&e.to_string()))
}
