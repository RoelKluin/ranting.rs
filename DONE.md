...
## Phases 3-6 — full record

Phases 3, 4, 5 and 6 were moved here verbatim from ROADMAP.md on 2026-08-14, replacing
the condensed summaries that used to stand in for them (those summaries had gone stale —
they still described the licensing decision as pending and `get_plural`/`get_singular` as
unwired). The sections below are the authoritative account.

Cross-references elsewhere in the repo cite these as "ROADMAP.md Phase N item M"; the
phase and item numbers are unchanged, only the file is.
...
6. ✅ **Hand-written placeholder tokenizer** (replaces `PH_EXT` regex internals)
   ...
   - This completes Phase 4's architecture-consolidation restructuring (items 1-6).
     ...
     [the historical license analysis](#historical-license-change-analysis-decided-implemented--kept-for-context). Item 8
...
itself. See [v1.3.0+](ROADMAP.md#v130-beyond-phase-6) below for where a
