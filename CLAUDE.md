...
- **Runtime tense selection (`say_with!()`)**: without a `NarrationContext.tense` override, `say_with!()` reproduces `say!()`'s output exactly for the same placeholder (falls back to the placeholder's own marker). Viewpoint/narration-person override (ROADMAP.md item 4) is not implemented yet — only tense.
